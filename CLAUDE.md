...
Base image project for the **coder** flavor — Coder IDE + SSH, minimal install. Produces `coder/base` image. Part of the [idekube-container](https://github.com/idekube-project/idekube-container) project.

## Build Commands

```bash
make prepare                  # Init submodules + create symlinks (artifacts, healthcheck, frontend)
make build                    # Build coder/base image locally
...
- **`.dockerargs.base`** — Build-time variables (BASE_IMAGE, CODER_VERSION, TINI_VERSION, etc.)
...
- **Stable tag**: After verification, `make tag-stable` retags the image. Derived images (`coder/conda`) `FROM` this stable tag.
